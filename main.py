...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        "--base",
        nargs="*",
        default="configs/ldm/ldmuie-vquie-f16-c256-cross.yaml", #  configs/autoencoder/vq_f16×16×3.yaml
        metavar="base_config.yaml",
        help="paths to base configs. Loaded from left-to-right. "
             "Parameters can be overwritten or added with command-line options of the form `--key value`.",
        #  configs/ldm/ldm.yaml  configs/ldm/ldmuie-vquie-f16-c256-cross.yaml
    )
    parser.add_argument(
        "-t",
        "--train",
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if log:
            ...
            if split == 'val':
                ...
                #gts = ((batch['input'][:N] + 1.0) / 2.0).mul(255).round().cpu() #enhance
                gts = ((batch['target'][:N] + 1.0) / 2.0).mul(255).round().cpu() #sr
...
if __name__ == "__main__":
    ...
    if opt.resume:
        ...
        #print("base_configs + opt.base =", base_configs, opt.base)
    ...
    try:
        ...
        if not cpu:
            ngpu = 1#len(lightning_config.trainer.devices.strip(",").split(','))
        else:
            ngpu = 1
